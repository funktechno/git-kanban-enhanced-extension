  ...
  methods: {
    ...
    addLabels: async function(issueID, labelIDs) {
      ...
      return this.$http.post(
        window.location.origin + `/api/v1/repos/${data}/issues/${issueID}/labels`,
        {
          labels: labelIDs,
        },
        {
    ...
    deleteLabel: async function(issueID, labelID) {
      ...
      return this.$http.delete(window.location.origin + `/api/v1/repos/${data}/issues/${issueID}/labels/${labelID}`, {
        cache: 'no-cache',
        headers: {
          Authorization: `token ${oToken}`,
          'Content-Type': 'application/json',

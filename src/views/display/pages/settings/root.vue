  ...
  <div>
    ...
    <div>
      ...
      <ul v-if="currentOptions && optionIndex !== -1 && currentOptions[optionIndex].repos && currentOptions[optionIndex].repos[repoName]">
        <li v-for="(stage, index) in currentOptions[optionIndex].repos[repoName].stages" v-bind:key="index">
          <div class="ui label has-emoji" v-bind:style="labelStyle(stage)" style="color: #000"><i class="octicon octicon-tag"></i> {{ stage }}</div>
    ...
    <label
      ...
      <div
        ...
        v-bind:style="{ 'background-color': '#' + availableLabels[labelIndex].color }"
        style="color: #000"
      >
    ...
    <select id="gitHost" v-model="labelIndex">
      <option v-for="(l, index) in availableLabels" v-bind:key="l.id" :value="index"
        ><span v-bind:style="{ color: '#' + l.color }">{{ l.name }}</span></option
      >
    </select>
    <br />
      ...
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          if (
            vm.currentOptions &&
            vm.optionIndex !== -1 &&
            vm.currentOptions[vm.optionIndex].repos &&
            vm.currentOptions[vm.optionIndex].repos[vm.repoName] &&
            vm.currentOptions[vm.optionIndex].repos[vm.repoName].stages &&
            vm.currentOptions[vm.optionIndex].repos[vm.repoName].stages.length > 0
          ) {
